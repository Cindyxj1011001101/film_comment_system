  ...
  <div>
    <div style="display: flex; align-items: flex-start; grid-gap: 10px">
      <div style="flex: 1; width: 0">
        <div class="card" style="padding: 20px; margin-bottom: 10px">
          <div style="display: flex; align-items: center;">
            <div style="font-weight: bold; font-size: 18px; flex: 1">{{ data.book.name }}</div>
            <div>
          ...
          <div style="margin-bottom: 20px">
            <el-rate v-model="data.book.score" disabled allow-half show-score></el-rate>
          </div>

          <div style="display: flex; grid-gap: 20px; margin-bottom: 20px">
            <img :src="data.book.cover" alt="" style="width: 200px; height: 270px; border-radius: 5px">
            <div style="flex: 1; color: #666">
              <div style="margin-bottom: 10px"><strong>作者：</strong>{{ data.book.author }}</div>
              <div style="margin-bottom: 10px"><strong>类型：</strong>{{ data.book.bookcategoryName }}</div>
              <div style="margin-bottom: 10px"><strong>国家/地区：</strong>{{ data.book.country }}</div>
              <div style="margin-bottom: 10px"><strong>语言：</strong>{{ data.book.language }}</div>
              <div style="margin-bottom: 10px"><strong>年份：</strong>{{ data.book.year }}</div>
              <div style="margin-bottom: 10px"><strong>出版日期：</strong>{{ data.book.date }}</div>
              <div style="margin-bottom: 10px"><strong>页数：</strong>{{ data.book.page }}</div>
            </div>
          </div>

          <div style="font-size: 20px; color: #1967e3; margin-bottom: 10px">{{ data.book.name }} 的剧情简介</div>
          <div style="color: #666; line-height: 24px; text-align: justify">{{ data.book.descr }}</div>
        </div>

        <div class="card" style="padding: 20px; margin-bottom: 10px">
          <div style="font-size: 20px; color: #239113; margin-bottom: 10px">{{ data.book.name }} 的短评 <span style="font-size: 14px; color: #333">...(全部{{ data.totalShort }}条)</span></div>
        ...
        <div class="card" style="padding: 20px">
          <div style="font-size: 20px; color: orange; margin-bottom: 10px">{{ data.book.name }} 的长评 <span style="font-size: 14px; color: #333">...(全部{{ data.totalLong }}条)</span></div>
      ...
      <div style="width: 300px; padding: 20px" class="card">
        <div style="font-size: 20px; margin-bottom: 20px">推荐书目</div>
...
request.get('/book/selectById/' + data.id).then(res => {
  data.book = res.data
})

request.get('/book/selectRecommend/' + data.id).then(res => {
