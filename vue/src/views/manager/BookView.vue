<template>
  <div>
    <div class="card" style="margin-bottom: 5px">
      <el-input v-model="data.name" placeholder="请输入书本名称查询" style="width: 300px; margin-right: 10px"></el-input>
      <el-button type="primary" @click="load">查询</el-button>
      <el-button type="info" @click="reset">重置</el-button>
    </div>

    <div class="card" style="margin-bottom: 5px">
      <el-button :class="{ 'book-active' : data.categoryId === null }" @click="loadBookByCategory(null)">全部</el-button>
      <el-button :class="{ 'book-active' : data.categoryId === item.id }" v-for="item in data.categoryList" :key="item.id" @click="loadBookByCategory(item.id)">{{ item.name }}</el-button>
    </div>

    <div class="card" style="margin-bottom: 5px">
      <el-row :gutter="20">
        <el-col :span="6" v-for="item in data.tableData" :key="item.id" style="margin-bottom: 20px; cursor: pointer" @click="goDetail(item.id)">
          <img :src="item.cover" alt="" style="width: 300px;height:400px;border-radius: 5px">
...
// 根据分类查询电影列表
const loadBookByCategory = (categoryId) => {
